...
object BeiDims {
     ...
     * 来源 / 功能大图标卡。
     *
     * 2026-09-21 从 44dp 收到 **28dp**（真机反馈"图标太大、一屏显示不下"）。
     * 真正撑爆一屏的其实是**卡片宽度**（240dp × 5 张 = 1272dp > 屏宽约 960dp，
     * 见 `NetworkRows.SourceCardWidth`）；图标跟着缩小，才不至于在 120dp 宽的卡里
     * 把标题挤出去。
     *
     * 定在 28dp 而不是更小：`BeiIconCard` 把描述改成"聚焦时才浮到卡片顶部"之后，
     * 卡片只剩「图标 + 标题」两行，高度有富余，没必要再压图标。
     * **别再往下收** —— 低于 20dp 时那几笔 0.10×宽 的描边就开始糊，三米外认不出符号。
     */
    val IconSourceCard = 28.dp
