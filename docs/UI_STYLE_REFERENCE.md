...
| 落地常量 | 值 | 用途 |
|---|---|---|
| `BeiDims.IconSourceCard` | **28dp**（圆角 ≈ 28% 边长） | 来源 / 功能大图标卡（`SourceIcon`）。**2026-09-21 由 44dp 收到 28dp**（真机反馈"图标太大、一屏显示不下"）。**别低于 20dp** —— 那几笔 0.10×宽 的描边会糊 |
...
> **一条必须先算的量法**：Android TV 上 **1dp = 2px**（xhdpi），所以 **1080p 屏的可用宽度
> 只有约 960dp**（还要再减掉外壳左右各 40dp 的安全边距）。网络主页原来把入口卡定宽写成
> `240dp`，5 张 = 240×5 + 间隙 18×4 = **1272dp**，第 4、5 张直接被挤出屏幕 ——
> 真机反馈的「一屏显示不下」**真正的原因是它，不是图标大小**。
> 现在 `NetworkRows.SourceCardWidth` = **120dp**（5 张共 672dp，约占七成宽，右边留白透气）。
> **新排一行卡片之前，先按 960dp 的预算算一遍再定宽。**
>
> **字号别跟着宽度一起减半**：`BeiIconCard` 从 240dp 收到 120dp 时，内边距与字距都收了
> 约一半，但**字号保持在电视的舒适下限**（`BodySize` = 13sp）；按字面"减半"会到 8.5sp，
> 三米外根本读不出。
>
> **描述与标题共用一行（2026-09-21）**：`BeiIconCard` 只有「图标 + 一行文字」——
> 那行平时是**标题**（"这张卡叫什么"），聚焦时换成**描述**并跑马灯（"它到底是什么"），
> **两者字体、字号、字重、颜色完全一致**。
>
> 这是第三版，前两版都被否了：第一版让描述"永远占一行、平时空白"（那一行 16dp 白养着）；
> 第二版让它浮在卡片顶部（又得在顶上预留一条内边距，同样白养）。现在直接复用标题那一行，
> 卡片比前两版都矮，省下的高度给了图标（24 → 28dp）。
>
> **描述必须用主标题的字号** —— 用小一号的字，聚焦瞬间这张卡会显得矮了一截、像换了一张卡；
> 同一套字体才只是"这一行的内容换了"。（实现见 `ui/shell/BeiUi.kt` 的 `BeiIconCard`。）
...
>
> **品牌图标（2026-09-21）**：`SourceIcon` 给云盘三家与 NAS 四家各画了**品牌轮廓**
> （夸克折角 / 百度熊掌 / 阿里折面三角 / 飞牛牛头 / 群晖开口环 / 威联通 Q / 绿联 U）。
> 三条纪律：
> 1. **一律香槟金线条 + 无色玻璃底**，不用品牌色 —— 面板不上色、强调色唯一；
> 2. **一个图标不超过 4 笔**：图标实际只有约 **14.6dp**（`size * 0.52f` —— 28dp 的图标画在 52% 的框里），笔画再多就是一团糊；
> 3. **不许撞脸**：三家网盘的标志核心本来都是"云"，可这一页已经有一个 WebDAV 的云，
>    所以三家一律改用**非云**的关键部件（理由写在 `SourceIcon` 的注释里）。
>
> 品牌 logo 属商标，这里只作**识别用途**的线条化仿制。若要规避，把对应分支换成
> **首字母构图**（夸 / 百 / 阿 / 牛 / 晖 / 通 / 联）即可，其余代码不用动。
...
**外壳侧（非播放页）的层序** —— 这一侧全部走**正常的 Compose 焦点**：

| 层（由内到外） | 返回键行为 |
|---|---|
| SMB / WebDAV / NAS 表单 | 关掉表单，回卡片行 |
| NAS 选品牌子页 / 云盘子页 / 「本地网络」子页 | 回网络主页 |
| 目录浏览（`RemoteBrowseScreen`） | 回上级目录；已在根目录则退出浏览 |
| **外壳首页**（三个标签之一） | **弹「退出应用？」确认框**（默认焦点「取消」） |
| 退出确认框打开时 | 关掉它（＝取消） |

确认框由 `AppRoot` 的 `BackHandler` 统管，组件是 `ui/shell/BeiUi.kt` 的 `BeiConfirmDialog`。
它**与播放页那个 `PlayerExitConfirmOverlay` 不共用**：播放页整屏是一个自管按键的
`focusable()`，按钮不在焦点系统里、高亮靠参数画出来；外壳是焦点驱动的，必须用真按钮
（`BeiPillButton`），否则遥控器推不动。
...
- [x] 图标：大卡片 **24dp**、顶部标签 22dp、状态 18dp；**同一排的外接尺寸统一**
- [x] 一行卡片的宽度总和 ≤ **960dp**（1080p 电视在 xhdpi 下的可用宽，见 §2.2 的量法）
...
| 网络位置（**4 个一级入口**：本地网络 / 云盘 / NAS / WebDAV） | `ui/network/NetworkScreen.kt`、`ui/network/NetworkRows.kt` |
| 「本地网络」子页（进入即扫 **SMB + DLNA 两种协议** + 「＋ 手动配置」） | `ui/network/LocalNetworkScreen.kt` |
| 「云盘」子页（夸克 / 百度 / 阿里，各带品牌轮廓图标） | `ui/network/CloudDriveScreen.kt` |
| 「NAS」子页 + 按品牌接入的表单（飞牛 / 群晖 / 威联通 / 绿联，各带品牌轮廓图标） | `ui/network/NasVendorScreen.kt`、`ui/network/NasServerForm.kt` |
